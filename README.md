...
`visualc\sdl_paths.props` have to point at SDKs built from source,
each holding an `include` directory and a `lib\arm64` directory.

The `windows-arm` job in `.github/workflows/build.yml` shows how to
build and stage them. ARM64 builds omit the x86-64-only MIDI backends.
...
macOS, Android, iOS, Switch, and Vita. The suite runs on both
architectures of both desktop systems. Get current builds from the
