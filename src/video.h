...
/* Sub-pixel supersampling: 0 = Auto, 1 = off, 2..5 = fixed, 6 = Native display factor. Native is
 * uncapped below RENDER_SUPERSAMPLE_LIMIT; Auto retains the ordinary ceiling. */
...
// Cursor-free frame scaled by JE_showVGA; centered menus use the pillarboxed composite.
...
// Present a supersampled 8-bit frame in the classic on-screen rectangle.
...
/* SDL window events use points; renderer output uses pixels. Size and convert with the
 * helpers below. The scale is 1 when the backend has no point/pixel split. */
