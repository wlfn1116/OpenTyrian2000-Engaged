...
Generated by `tools/dump/dump_data.py` from a Tyrian 2.1 data directory. Every
...
| `images/pic/` | The 13 full-screen backdrops in `tyrian.pic`, each with the palette it ships with. |
| `images/pcx/` | The eight stand-alone PCX screens, which carry their own palettes. |
| `images/icons/` | The Windows icons, copied and converted to PNG. |
| `sprites/tyrian_shp/` | The 12 banks in `tyrian.shp`: fonts, faces, planets, menu art, shots and ships. |
...
| `sprites/standalone/` | The stand-alone sprite files: `estsc.shp`, `user1.shp` and `user2.shp`. |
...
Episodes 1 to 3 share the item and enemy tables stored behind the text in
